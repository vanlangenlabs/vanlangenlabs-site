...
def add_label(image: Image.Image, label: str) -> Image.Image:
    ...
    text_y = (
        (LABEL_HEIGHT - (bounds[3] - bounds[1])) // 2
        - bounds[1]
    )

    draw.text(
        ((QR_SIZE - text_width) // 2, text_y),
        label,
        fill="black",
        font=font
    )
...
def save_qr_png(
    ...
    # Original RGB PNG output.
    # No palette conversion or color reduction.
...
def main() -> None:
    vcard_text = VCARD_PATH.read_text(
        encoding="utf-8"
    ).strip()

    compact_vcard = remove_photo(vcard_text)

    # Offline QR: complete vCard content.
    save_qr_png(
        compact_vcard,
        VCARD_QR_PATH
    )

    # Online QR: URL to the vCard.
    save_qr_png(
        VCARD_URL,
        VCARD_URL_QR_PATH
    )
